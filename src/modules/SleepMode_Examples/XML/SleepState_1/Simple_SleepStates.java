 ...
 * requirements.
 */
public class Simple_SleepStates {
    ...
    private static PowerDatacenterSimpleACPIEnergySavingStrategy createDatacenter(String name) throws Exception {
        ...
        int bw; // bandwith
        String acpiEnergySavingStrategy;
        ...
        for (int i = 0; i < hostsNumber; i++) {
            ...
            acpiEnergySavingEnable = tmp_host.isACPIEnergySavingEnable();
            acpiEnergySavingStrategy = tmp_host.getACPIEnergySavingStrategy();
            HashMap<String, ACPIStateDatas> acpiConfig  = tmp_host.getAcpiConfig();
            ...
            for (int pe = 0; pe < nb_pe; pe++) {
                peList.add(new Pe(pe, new PeProvisionerSimple(mips), freqs, govs.get(0), ConfigDvfs));
            }
            hostList.add(
                    new PowerHost(
                            i,
                            ...
                            false,
                            enableDVFS,
                            acpiEnergySavingEnable,
                            acpiConfig,
                            acpiEnergySavingStrategy)
            ); 
        }
        ...
        PowerDatacenterSimpleACPIEnergySavingStrategy powerDatacenter = null;
        try {
            powerDatacenter = new PowerDatacenterSimpleACPIEnergySavingStrategy(
                    name,
                    characteristics,
