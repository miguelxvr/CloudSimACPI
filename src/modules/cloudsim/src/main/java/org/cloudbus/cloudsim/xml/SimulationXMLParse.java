 ...
 * Toulouse, France Please cite: T. Guérout et al., Energy-aware simulation with
 * DVFS, Simulat. Modell. Pract. Theory (2013),
...
public class SimulationXMLParse {
    ...
    public HostDatas HOSTS_Parse(Element Host) {
        ArrayList<String> atts = new ArrayList<String>(Arrays.asList("ram", "storage", "bw", "maxP", "staticPP", "cpus", "mips", "onoff", "dvfsenable", "cpufrequencies", "dvfs", "acpiEnergySavingEnable", "acpiEnergySavingStrategy", "acpiConfig"));
        ...
        while (iterator.hasNext()) {
            ...
            switch (ind) {
                ...
                case 11:
                    tmp_host_datas.setACPIEnergySavingEnable(Boolean.parseBoolean(tmp_value));
                case 12:
                    tmp_host_datas.setACPIEnergySavingStrategy(tmp_value);
                case 13:
                    tmp_host_datas.setAcpiConfig(ACPIConfig_PARSE(tmp_e));
            }
        }
    ...
    public HashMap<String, ACPIStateDatas> ACPIConfig_PARSE(Element acpiConfig) {

        HashMap<String, ACPIStateDatas> tmp_ACPIState_Datas = new HashMap<>();
        List liste_Mode = acpiConfig.getChildren();

        int nb_Mode = liste_Mode.size();
        String tmp_name = "";

        System.out.println("nb mode == " + nb_Mode);
        for (int i = 0; i < nb_Mode; i++) {
        ACPIStateDatas tmp_ACPIState_Data = new ACPIStateDatas();

            Element One_state = (Element) liste_Mode.get(i);
            Iterator iterator = One_state.getChildren().iterator();

            Element e_name_mode = (Element) iterator.next();
            System.out.println("Name mode : " + e_name_mode.getValue());

            if (e_name_mode.getName().equalsIgnoreCase("name")) {
                tmp_name = e_name_mode.getValue();
            } else if (e_name_mode.getName().equalsIgnoreCase("power")) {

                while (iterator.hasNext()) {
                    Element tmp_e = (Element) iterator.next();

                    if (tmp_e.getName().equalsIgnoreCase("entering")) {
                        tmp_ACPIState_Data.setPower_entering(Integer.parseInt(tmp_e.getValue()));
                    } else if (tmp_e.getName().equalsIgnoreCase("leaving")) {
                        tmp_ACPIState_Data.setPower_leaving(Integer.parseInt(tmp_e.getValue()));
                    } else if (tmp_e.getName().equalsIgnoreCase("staying")) {
                        tmp_ACPIState_Data.setPower_staying(Integer.parseInt(tmp_e.getValue()));
                    }
                    System.out.println(tmp_e.getName() + " " + tmp_e.getValue() + " // ");
                }
            } else if (e_name_mode.getName().equalsIgnoreCase("time")) {

                while (iterator.hasNext()) {
                    Element tmp_e = (Element) iterator.next();

                    if (tmp_e.getName().equalsIgnoreCase("entering")) {
                        tmp_ACPIState_Data.setTime_entering(Integer.parseInt(tmp_e.getValue()));
                    } else if (tmp_e.getName().equalsIgnoreCase("leaving")) {
                        tmp_ACPIState_Data.setTime_leaving(Integer.parseInt(tmp_e.getValue()));
                    } else if (tmp_e.getName().equalsIgnoreCase("staying")) {
                        tmp_ACPIState_Data.setTime_staying(Integer.parseInt(tmp_e.getValue()));
                    }
                    System.out.println(tmp_e.getName() + " " + tmp_e.getValue() + " // ");
                }
            }
            tmp_ACPIState_Datas.put(tmp_name, tmp_ACPIState_Data);
        }
        return tmp_ACPIState_Datas;
    }
