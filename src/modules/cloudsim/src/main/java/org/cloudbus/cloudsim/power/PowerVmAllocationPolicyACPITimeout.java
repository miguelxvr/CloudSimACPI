...
public class PowerVmAllocationPolicyACPITimeout extends PowerVmAllocationPolicyAbstract {

    public PowerVmAllocationPolicyACPITimeout(List<? extends Host> list) {
        super(list);
    }
    ...
    public boolean allocateHostForVm(Vm vm) {
        ...
        if (ChoosenHost == null) {
            for (PowerHost host : this.<PowerHost>getHostList()) {
                if (host.getACPIState().equalsIgnoreCase("S3") && host.getIndexState() == PowerHost.ACPI_STAYING) {
                    ChoosenHost = host;
                    break;
                }
            }
